...
Read `${BRAIN_SOURCE:-~/source/bunny_stack}/registry.yml`. Match each action item to a project. Items that cannot be matched are marked "unassigned".
...
After confirmation:
1. Create `${BRAIN_SOURCE:-~/source/bunny_stack}/meetings/YYYY-MM-DD.md` (structured note)
...
5. `cd ${BRAIN_SOURCE:-~/source/bunny_stack} && git add . && git commit -m "brain: meeting [YYYY-MM-DD]" && git push`
