...
| Name | Path (小龙虾) | Notes |
|------|--------------|-------|
| registry.yml | `${BRAIN_SOURCE:-~/source/bunny_stack}/registry.yml` | project registry |
| external-brain root | `${BRAIN_SOURCE:-~/source/bunny_stack}/` | git repo root |
| workspace config | `${BRAIN_SOURCE:-~/source/bunny_stack}/workspace/` | this file's directory |
...
|----------|---------|---------|
| `$BRAIN_SOURCE` | Location of this external-brain repo on the deployment machine. Used to find `registry.yml` and all project brain docs. Falls back to `~/source/bunny_stack` if unset. | `~/source/bunny_stack` |
